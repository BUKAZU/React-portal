...
describe('ObjectDetails', () => {
  ...
  it('should render the component with correct structure', () => {
    ...
    expect(arrivalCells?.[0].textContent).toBe(
      FormatIntl(Parse_EN_US(mockValues.arrivalDate.date), LONG_DATE_FORMAT)
    );
    ...
    expect(departureCells?.[0].textContent).toBe(
      FormatIntl(Parse_EN_US(mockValues.departureDate.date), LONG_DATE_FORMAT)
    );
  ...
  it('should call formatMessage with correct translation keys', () => {
    ...
    expect(mockFormatMessage).toHaveBeenCalledWith({
      id: 'test_house.arrival'
    });
    expect(mockFormatMessage).toHaveBeenCalledWith({
      id: 'test_house.departure'
    });
  });
  ...
  it('should handle missing image URL', () => {
    ...
    const detailsWithoutImage = new ObjectDetails(
      houseWithoutImage as any,
      mockValues as any
    );
